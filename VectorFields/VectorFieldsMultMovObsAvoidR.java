 ...
 */
public class VectorFieldsMultMovObsAvoidR extends AdvancedRobot
{
    ...
    public void run()
    {
        ...
        while (true)
        {
            if (foundGoal)
            {
				...
				if (speedToGoal == 0 || getVelocity() == 0) {
					System.out.println("idlin...");
					++idle;
				}
				if (idle == 20) {
					System.out.println("guess i'm stuck");
				// random field
					double randSpeed = Math.random() * MAX_SPEED;
					double randTurn = Math.random() * 360;
					speedToGoal += 5;//randSpeed;
					adjustment += 30;//randTurn;
					idle = 0;
				}
			...
			if (getVelocity() == 0) {
				...
				++idle;
				if (idle > 25) {
					double randSpeed = Math.random() * 25;
					double randTurn = Math.random() * 360;
					setTurnLeft(randTurn);
					setAhead(randSpeed);
					idle = 0;
				}
			}
