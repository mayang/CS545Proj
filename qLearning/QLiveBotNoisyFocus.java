...
;	public void run() {
		currently_updating = false;
    	 while (true) {
        	...
        	int state = QUtilities2.XYtoState(getX(), getY());
        	if (policy[state] == QUtilities2.ACTION_NORTH) {
        		goNorth(10);
        	} else if (policy[state] == QUtilities2.ACTION_SOUTH) {
        		goSouth(10);
        	} else if (policy[state] == QUtilities2.ACTION_EAST) {
        		goEast(10);
        	} else if (policy[state] == QUtilities2.ACTION_WEST) {
        		goWest(10);
        	} else if (policy[state] == QUtilities2.ACTION_NORTHWEST) {
        		goNorthwest(10);
        	} else if (policy[state] == QUtilities2.ACTION_NORTHEAST) {
        		goNortheast(10);
        	} else if (policy[state] == QUtilities2.ACTION_SOUTHWEST) {
        		goSouthwest(10);
        	} else if (policy[state] == QUtilities2.ACTION_SOUTHEAST) {
        		goSoutheast(10);
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
    	...
    	goal_state = QUtilities2.XYtoState(enemyX, enemyY);
    	updatePolicy();
        fire(1);
	}
    ...
    public void updatePolicy() {
    	if (!currently_updating) {
    		...
    		Thread policy_update = new Thread() {
    			public void run() {
    					policy = QUtilities2.QtoPolicy(QUtilities2.generateQTable(goal_state, QUtilities2.XYtoState(getX(), getY())));
    					doneUpdating();
			    	}
				};
			policy_update.start();
