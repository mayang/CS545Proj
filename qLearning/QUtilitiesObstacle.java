...
public class QUtilitiesObstacle {
...
//	public static  int[] OBSTACLES = {200,10,200,30,200,50,200,70,200,90,200,110,200,130,200,150,200,170,200,190};
	public static  int[] OBSTACLES = {};
	public static Set<Integer> OBSTACLE_STATES = obstacleXYsToStates(OBSTACLES);
	...
	public static int[] QtoPolicy(double[][] QTable) {
		...
		for(int i=0; i< QUtilitiesObstacle.NUM_STATES; i++) {
	...
	public static double[][] generateQTable(int goal, int current) {
		double[][] Q = new double[QUtilitiesObstacle.NUM_STATES][QUtilitiesObstacle.NUM_ACTIONS];
		for(int episode = 0; episode < NUM_EPISODES; episode++) {
			int T = 11;
			int s;
			if (current == -1) {
				s = (int)Math.random() * QUtilitiesObstacle.NUM_STATES;
			} else {
				s = current;
			}
			int sNext;
...
//				a = epsilonPickAction(s, Q, QUtilities.epsilon);
	...
	public static void setObstacles(int[] obstaclesXY) {
	...
	public static void updateQ(double[][] QTable, int curState, int curAction, double r, int nextState){
		...
		QTable[curState][curAction] = curQ + (QUtilitiesObstacle.alpha * (r + QUtilitiesObstacle.gamma * (maxAQ(nextState, QTable)- curQ)));
	}
	
	public static int maxA(int state, double[][] Q) {
		int maxA = -1;
		double curMax = -1000000; 
		for(int j=0; j<QUtilitiesObstacle.NUM_ACTIONS; j++) {
			if (Q[state][j] > curMax) {
				maxA = j;
				curMax = Q[state][j];
			}
	...
	public static double maxAQ(int nextState, double[][] QTable){
		double maxQ = -1000;
		for (int i=0; i < QUtilitiesObstacle.NUM_ACTIONS; i++) {
	...
	public static int transitionResult(int state, int action){
		...
		switch(action){
		case QUtilitiesObstacle.ACTION_NORTH:
			if (! nextToTopWall(state)) {
				trialNextState = state + QUtilitiesObstacle.ROW_LENGTH;
				break;
			}else {
				return state;
			}
		case QUtilitiesObstacle.ACTION_NORTHEAST:
			if (! nextToTopWall(state) && ! nextToRightWall(state)) {
				trialNextState = state + QUtilitiesObstacle.ROW_LENGTH + 1;
				break;
			} else {
				return state;
			}
		case QUtilitiesObstacle.ACTION_EAST:
		...
		case QUtilitiesObstacle.ACTION_SOUTHEAST:
		...
		case QUtilitiesObstacle.ACTION_SOUTH:
		...
		case QUtilitiesObstacle.ACTION_SOUTHWEST:
		...
		case QUtilitiesObstacle.ACTION_WEST:
		...
		case QUtilitiesObstacle.ACTION_NORTHWEST:
	...
	public int randomAction() {
		return (int) Math.random() * QUtilitiesObstacle.NUM_ACTIONS;
	}
