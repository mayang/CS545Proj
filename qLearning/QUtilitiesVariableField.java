...
public class QUtilitiesVariableField {
	...
	public static int[] QtoPolicy(double[][] QTable) {
		...
		for(int i=0; i< QUtilitiesVariableField.NUM_STATES; i++) {
	...
	public static double[][] generateQTable(int goal, int current, int rowLength, int episodes) {
		...
		double[][] Q = new double[QUtilitiesVariableField.NUM_STATES][QUtilitiesVariableField.NUM_ACTIONS];
		for(int episode = 0; episode < episodes; episode++) {
			int T = 11;
			int s;
			if (current == -1) {
				s = (int)Math.random() * QUtilitiesVariableField.NUM_STATES;
			} else {
				s = current;
			}
			int sNext;
...
//				a = epsilonPickAction(s, Q, QUtilities.epsilon);
	...
	public static void updateQ(double[][] QTable, int curState, int curAction, double r, int nextState){
		...
		QTable[curState][curAction] = curQ + (QUtilitiesVariableField.alpha * (r + QUtilitiesVariableField.gamma * (maxAQ(nextState, QTable)- curQ)));
	}
	
	public static int maxA(int state, double[][] Q) {
		int maxA = -1;
		double curMax = -1000000; 
		for(int j=0; j<QUtilitiesVariableField.NUM_ACTIONS; j++) {
			if (Q[state][j] > curMax) {
				maxA = j;
				curMax = Q[state][j];
			}
	...
	public static double maxAQ(int nextState, double[][] QTable){
		double maxQ = -1000;
		for (int i=0; i < QUtilitiesVariableField.NUM_ACTIONS; i++) {
	...
	public static int transitionResult(int state, int action){
		switch(action){
		case QUtilitiesVariableField.ACTION_NORTH:
			if (! nextToTopWall(state)) {
				return state + QUtilitiesVariableField.ROW_LENGTH;
			}else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_NORTHEAST:
			if (! nextToTopWall(state) && ! nextToRightWall(state)) {
				return state + QUtilitiesVariableField.ROW_LENGTH + 1;
			} else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_EAST:
			if (! nextToRightWall(state)) {
				return state + 1;
			} else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_SOUTHEAST:
			if (! nextToBottomWall(state) && ! nextToRightWall(state)) {
				return state - QUtilitiesVariableField.ROW_LENGTH + 1;
			} else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_SOUTH:
			if (! nextToBottomWall(state)) {
				return state - QUtilitiesVariableField.ROW_LENGTH;
			} else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_SOUTHWEST:
			if (! nextToLeftWall(state) && ! nextToBottomWall(state)) {
				return state - QUtilitiesVariableField.ROW_LENGTH -1;
			} else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_WEST:
			if (! nextToLeftWall(state)) {
				return state -1;
			} else {
				return state;
			}
		case QUtilitiesVariableField.ACTION_NORTHWEST:
	...
	public int randomAction() {
		return (int) Math.random() * QUtilitiesVariableField.NUM_ACTIONS;
	}
