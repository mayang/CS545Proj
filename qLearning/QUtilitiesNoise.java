...
public class QUtilitiesNoise {
	...
	public static int[] QtoPolicy(double[][] QTable) {
		...
		for(int i=0; i< QUtilitiesNoise.NUM_STATES; i++) {
	...
	public static double[][] generateQTable(int goal, int current) {
		double[][] Q = new double[QUtilitiesNoise.NUM_STATES][QUtilitiesNoise.NUM_ACTIONS];
		for(int episode = 0; episode < NUM_EPISODES; episode++) {
			int T = 11;
			int s;
			if (current == -1) {
				s = (int)Math.random() * QUtilitiesNoise.NUM_STATES;
			} else {
				s = current;
			}
			int sNext;
...
//				a = epsilonPickAction(s, Q, QUtilities.epsilon);
	...
	public static void updateQ(double[][] QTable, int curState, int curAction, double r, int nextState){
		...
		QTable[curState][curAction] = curQ + (QUtilitiesNoise.alpha * (r + QUtilitiesNoise.gamma * (maxAQ(nextState, QTable)- curQ)));
	}
	
	public static int maxA(int state, double[][] Q) {
		int maxA = -1;
		double curMax = -1000000; 
		for(int j=0; j<QUtilitiesNoise.NUM_ACTIONS; j++) {
			if (Q[state][j] > curMax) {
				maxA = j;
				curMax = Q[state][j];
			}
	...
	public static double maxAQ(int nextState, double[][] QTable){
		double maxQ = -1000;
		for (int i=0; i < QUtilitiesNoise.NUM_ACTIONS; i++) {
	...
	public static int transitionResult(int state, int action){
		switch(action){
		case QUtilitiesNoise.ACTION_NORTH:
			if (! nextToTopWall(state)) {
				return state + QUtilitiesNoise.ROW_LENGTH;
			}else {
				return state;
			}
		case QUtilitiesNoise.ACTION_NORTHEAST:
			if (! nextToTopWall(state) && ! nextToRightWall(state)) {
				return state + QUtilitiesNoise.ROW_LENGTH + 1;
			} else {
				return state;
			}
		case QUtilitiesNoise.ACTION_EAST:
			if (! nextToRightWall(state)) {
				return state + 1;
			} else {
				return state;
			}
		case QUtilitiesNoise.ACTION_SOUTHEAST:
			if (! nextToBottomWall(state) && ! nextToRightWall(state)) {
				return state - QUtilitiesNoise.ROW_LENGTH + 1;
			} else {
				return state;
			}
		case QUtilitiesNoise.ACTION_SOUTH:
			if (! nextToBottomWall(state)) {
				return state - QUtilitiesNoise.ROW_LENGTH;
			} else {
				return state;
			}
		case QUtilitiesNoise.ACTION_SOUTHWEST:
			if (! nextToLeftWall(state) && ! nextToBottomWall(state)) {
				return state - QUtilitiesNoise.ROW_LENGTH -1;
			} else {
				return state;
			}
		case QUtilitiesNoise.ACTION_WEST:
			if (! nextToLeftWall(state)) {
				return state -1;
			} else {
				return state;
			}
		case QUtilitiesNoise.ACTION_NORTHWEST:
	...
	public int randomAction() {
		return (int) Math.random() * QUtilitiesNoise.NUM_ACTIONS;
	}
