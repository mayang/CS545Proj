...
//		try {
		...
		for(int episode = 0; episode < QUtilities.NUM_EPISODES; episode++) {
			int T = 11;
...
//				System.out.println(duration);
	...
	private static void updateQ(double[][] QTable, int curState, int curAction, int curReward, int nextState){
		...
		QTable[curState][curAction] = curQ + (QUtilities.alpha * (curReward + QUtilities.gamma * (maxAQ(nextState, QTable)- curQ)));
	}
